...
* 目标：在 `core.py` 按 `jiagou.tex` 的 OG(t) 公式，建立三类效用并记录/作图：移动机器人效用 U\_i^a(t)、服务器效用 U\_j^i(t)、总目标 OG(t)。

* 公式映射：

  * 卸载决策 φ\_i^a(t)：`a∈{0,E_j}`，本地 `a=0`；卸载到服务器 j 则 `a=E_j`。来源：`agent.action.offload`，本地为全 0；卸载为 one-hot 指向 j。

  * 权重/功率 p\_i^b(t)：使用当前实现中的 `p_weight` 映射：本地 `p_weight=1.0`，边缘 `p_weight=agent.state.trans_pow`。与 `environment.py:111-114` 的 `objective_cur=p_weight*(E+λT)` 一致。

  * U\_i^a(t)：采用“满意度−成本”的规范化组合：`U_i^a(t)=θ·S_norm − (1−θ)·C_norm`，θ∈\[0,1]。

  * U\_j^i(t)：服务器侧对任务 i 的效用，结合完成/失败增量与负载/紧急度。

## 移动机器人效用 U\_i^a(t)

* 满意度 S\_norm：基于截止约束的时延满意度

  * `S_norm = clamp(1 − T_cur / τ, 0, 1)`

  * 变量：`T_cur=agent.state.time_cur`，`τ=agent.state.task_delay_tol`；若 `τ≤0`，则 `S_norm=0`。

* 成本 C\_norm：按用户要求“归一化成本为每时隙服务器分配给该任务的计算资源”
