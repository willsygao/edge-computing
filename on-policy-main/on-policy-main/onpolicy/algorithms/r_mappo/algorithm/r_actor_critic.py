...
class R_Critic(nn.Module):
    ...
    def forward(self, cent_obs, rnn_states, masks):
        ...
        # 应用注意力机制
        query = key = value = critic_features.unsqueeze(1)  # 增加时间维度
        # 假设 actor_features 包含了所有观测信息
        # 提取智能体自身的状态作为 query
        # query = critic_features[:, :self.self_state_dim].unsqueeze(1)  # 假设前 self_state_dim 维是自身状态
        # 使用所有观测信息作为 key 和 value
        # key = value = critic_features.unsqueeze(1)
